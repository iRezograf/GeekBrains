...
namespace TASK47
{
    public class Arrays
    {
        ...
        public void CreateArray(int count, int minVal, int maxVal, int [] result)
        {
            Random ran = new Random();
        ...
        public void CreateArray(int count, int minVal, int maxVal, double [] result)
        {
            Random ran = new Random();
            for (int i = 0; i < result.Length; i++)
            {
                result[i] = ran.Next(minVal, maxVal + 1)/10.0;
            }
        }
        ...
        public void CreateArray(int row, int column, int minVal, int maxVal, out double [,] array)
        {
            array = new double[row, column];

            double [] columns = new double [column]; // = new int[column];
            for (int i = 0; i < row; i++)
            {
                CreateArray(column, minVal, maxVal + 1, columns);
                for (int j = 0; j < column; j++)
                {
                    array[i , j] = columns[j];
                }
